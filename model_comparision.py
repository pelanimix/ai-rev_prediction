...
MODEL_DIR = "models"
MODEL_VERSION = 0.1
MODEL_VERSION_NOTE = "supervised learing model for time-series - compare multiple models"
...
def _model_train_forest(df,tag,test=False):
    ...
    grid = GridSearchCV(pipe_rf, param_grid=param_grid_rf, cv=5, n_jobs=-1) #iid=False
...
def _model_train_gradient_boost(df,tag,test=False):
    ...
    grid = GridSearchCV(pipe_gb, param_grid=param_grid_gb, cv=5, n_jobs=-1) #iid=False
    grid.fit(X_train, y_train)
    y_pred = grid.predict(X_test)
    eval_mse =  round(mean_squared_error(y_test,y_pred))
    eval_rmse =  round(np.sqrt(mean_squared_error(y_test,y_pred)))
